...
bool Application::update(int deltaTime)
{
    ...
    if (keys[119]) scene.getCamera().moveCamera(0.016f * camera_speed, 0.0f); /* w */
    if (keys[115]) scene.getCamera().moveCamera(0.016f * (-camera_speed), 0.0f); /* s */
    if (keys[97]) scene.getCamera().moveCamera(0, 0.016f * (-camera_speed)); /* a */
    if (keys[100]) scene.getCamera().moveCamera(0, 0.016f * camera_speed); /* d */
...
void Application::SetRenderingTime(double secs) {

    scene.SetRenderingTime(secs);
